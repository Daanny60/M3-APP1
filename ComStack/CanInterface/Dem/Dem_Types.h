...
#define DTC_NOSELFRECOVER  (0u)
#define DTC_SELFRECOVER  (1u)
/*------------------------------------------------------------------------------------------M3 状态位
---------------------------------------------------------------------------------------------*/
typedef union
{
	uint8_t status_byt;
  	struct{
		unsigned char test_failed                       :1;		//用以確定故障是否存在11111
		unsigned char test_failed_this_operation_cycle     :1;	//在這周期中，任何失敗的測試都曾發生
		unsigned char pending_dtc                       :1;		//等待DTC  故障状态--当test至少通过一次且从未发生故障时，
															//在一個操作周期完成後才會清除。
		unsigned char confirmed_dtc                     :1;		//确认DTC   1111 
		...
		unsigned char warning_indicator_requested        :1;		//當警告指示是需要的      111111   
	} status_bits;
} dtc_status_t;
...
typedef struct{
	...
	uint16_t c_main_ecu_vehicle_speed;

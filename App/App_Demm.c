...
#define APP_DEMM_PERIOD_TIME_MS  10
#define BCM_DTC_TIME_500MS    (500/APP_DEMM_PERIOD_TIME_MS)
#define EMS_DTC_TIME_250MS    (250/APP_DEMM_PERIOD_TIME_MS)

uint8_t  EMS_dtc_fail_clear_cnt = 0;
uint8_t  BCM_dtc_fail_clear_cnt = 0;

uint8_t EMS_dtc_fail_flag = 0;
uint8_t BCM_dtc_fail_flag = 0;
...
static inline void  AppDemm_BCM_Timeout_FailureProcess(void)
{
    DemmDtcEvent_e  event;
    event = AppDemm_BCM_Timeout_Failure_Event_Get();
    if (event == DTC_TEST_EVENT_FAILED)
    {
...
static inline void  AppDemm_EMS_Timeout_FailureProcess(void)
{
    DemmDtcEvent_e  event;
    event = AppDemm_EMS_Timeout_Failure_Event_Get();
    if (event == DTC_TEST_EVENT_FAILED)
    {
...
void App_Demm_Task(void)
{
    ...
    if (rx_timeout_flag & COM_RXIPDU_Message2_TIMEOUTFLAG == 0x2)
    {
        ...
        AppDemm_EMS_Timeout_Failure_Event_Set(DTC_TEST_EVENT_FAILED);
    }
    else
    {
        if (EMS_dtc_fail_flag == 1)
        {
            if (EMS_dtc_fail_clear_cnt < EMS_DTC_TIME_250MS)
            {
                EMS_dtc_fail_clear_cnt++;
            }
            else
            {
                EMS_dtc_fail_flag = 0;
                EMS_dtc_fail_clear_cnt = 0;
                AppDemm_EMS_Timeout_Failure_Event_Set(DTC_TEST_EVENT_PASSED);
            }
        }
        else
        {
    ...
    {
        if (BCM_dtc_fail_flag == 1)
        {
            if (BCM_dtc_fail_clear_cnt < BCM_DTC_TIME_500MS)
            {
                BCM_dtc_fail_clear_cnt++;
            }
            else
            {
                BCM_dtc_fail_flag = 0;
                BCM_dtc_fail_clear_cnt = 0;
                AppDemm_BCM_Timeout_Failure_Event_Set(DTC_TEST_EVENT_PASSED);
            }
        }
        else
        {

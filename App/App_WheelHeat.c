...
uint8_t vHeatDiag(void) {
    ...
    /*IS短路*/
    if (App_stWheelHeat.u16OnValue > ADC_WHL_HEAT_SHORT_THRESD) {
    ...
    /*IS开路*/
    if (App_stWheelHeat.u16OnValue < ADC_WHL_HEAT_OPEN_THRESD) {
...
void App_WheelHeat_Task(void) {
    ...
    do {
        ...
        if (((self_check_state & 0xC) >> 2) == 0) { //检查发动机状态连续3次都不是RUNNING
            self_check_cnt = 0;
            heat_diag_state = (heat_diag_state & (uint8_t)(~0x20)) | 0x20;
        } else {
            heat_diag_state &= (uint8_t)(~0x20);
    ...
    do {
        ...
        if (SWHMFailure_time_cnt) {
            SWHMFailure_time_cnt--;
            break;
        }
        if (SWHMFailure_tx_cnt < 100) {
